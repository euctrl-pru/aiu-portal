...
#folder_destination
dest_dir_root <- glue(
  "C:/Users/{username}/OneDrive - EUROCONTROL/Download data files"
)
...
last_month_beg <- today(tzone = "UTC") |>
  floor_date("month") |>
  `%m+%`(months(-1))


#---------- co2 emissions ----
# NOTE: this is done in emissions.Rmd
  ...
  group_walk(
    ~ write_csv(
      ...
      fs::path(
        ...
        dest_folder,
        stringr::str_glue(
          "vertical_flight_efficiency_{YYYY}.csv",
          YYYY = .y$YEAR
        )
      ),
      na = ""
  ...
  group_walk(
    ~ write_csv(
      ...
      fs::path(
        ...
        dest_folder,
        stringr::str_glue(
          "horizontal_flight_efficiency_{YYYY}.csv",
          YYYY = .y$YEAR
        )
      ),
      na = ""
...
export_airport_traffic(wef = "2024-01-01") |>
  dplyr::arrange(YEAR, MONTH_NUM, FLT_DATE, STATE_NAME, APT_ICAO) |>
  group_by(YEAR) |>
  group_walk(
    ~ write_csv(
      .x,
