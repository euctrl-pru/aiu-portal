...
The figure below presents the EUROCONTROL 7-year forecast (2023-2029) for air traffic movements (IFR) in millions of flights (Autumn Edition 2023). The information is available by [traffic zone (TZ)](https://ansperformance.eu/definition/traffic-zone/) and includes actual traffic up to 2022 and different forecast scenarios (high, base, low) up to 2029. Simply select the TZ you are interested in from the drop-down menu to see the detailed information for each year.
...
data <- read_csv2(here("content", "traffic", "eurocontrol-forecast-2023-2029-traffic-table-october2023-unroundedUPDATE.csv"))
  ...
  buttons = lapply(1:length(tz_values), function(i) {
    list(method = "update",
         args = list(
           ...
           list(title = list(
             text = paste0("<b>EUROCONTROL 7-year forecast for ", tz_values[i], " 2023-2029 </b> <br><span style='font-size:0.8em;color:grey'>Actual and future IFR movements</span>")
           ))
         ),
         label = tz_values[i])
  })
...
plot <-
  plot %>% layout(
    title = list(
      text = paste0("<b>EUROCONTROL 7-year forecast for ", tz_values[1], " 2023-2029 </b> <br><span style='font-size:0.8em;color:grey'>Actual and future IFR movements</span>")
    ),
    xaxis = list(
      title = "",
      tick0 = 2019,
...
For more information about the forecasts and the associated data, see the [EUROCONTROL Forecast Update 2023-2029 publication](https://www.eurocontrol.int/publication/eurocontrol-forecast-update-2023-2029-autumn-2023).
