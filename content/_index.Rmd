---
title: "Aviation Intelligence Unit Portal"
date: "2018-02-07T11:26:51+01:00"
---

<style type="text/css">
  .img-responsive {
      margin: 0 auto;
      max-width: 75%
  }
  
  .resp-container {
      position: relative;
      overflow: hidden;
      padding-top: 50.0%;
  }
  
  .resp-iframe {
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 110%;
      border: 0;
  }
      
  .Scorecard_subt {
    margin: 0px; 
    padding-bottom: 0px;  
    text-align: left; 
    color:#7f7f7f
  }

  .Scorecard_net {
    margin: 0px; 
    padding-bottom: 0px;
    font-weight: bold;  
    text-align: left;
    color: #0070c0; 
    
  }


.col-4 {
  margin: 0px;
  padding: 0px;
  text-align: right;
}
.w3-container {
  border-radius: 10px; 
}

  </style>

<!-- Daily traffic graphs and scorecards Scorecard_net-->
<div class="w3-container" >
<div class="row" style="padding: 0px; background-color: #d9e1f2; border-radius: 10px 10px 0 0; width: 100%;">
<div class="col-12"; style="margin: 0px; ">
<p style="font-weight: bold; color: #0070c0;">Latest network traffic situation: <span id="y2d_day_text">N/A</span></p>
</div>
</div>
</div>

<div class="w3-container">
<div class="row" style="padding: 0px; background-color: #d9e1f2; border-radius: 0 0 10px 10px; width: 100%;">

<div class="col">
  <p class="Scorecard_subt">Total flights</p>
  <p id="y2d_traffic" style="font-weight: bold; color: #0070c0;">n/a</p> 
</div>

<div class="col">
  <p class="Scorecard_subt">Avg. daily flights</p>
  <p id="avg_y2d_traffic" style="font-weight: bold; color: #0070c0;">n/a</p>
</div>

<div class="col-2" ; style="margin: 0px; ">
  <p class="Scorecard_subt">Vs 2019</p>
  <p id="dif_y2d_2019" style="font-weight: bold; color: #0070c0;">n/a</p>
</div>

<div class="col-2" ; style="margin: 0px;  ">
  <p class="Scorecard_subt">Vs prev. year</p>
  <p id="dif_y2d_prev_year" style="font-weight: bold; color: #0070c0;">n/a</p>
</div>


<div class="col" ; style="margin: 0px;  ">
</div>
<div class="col" ; style="align=center">
<!--<a href="https://www.eurocontrol.int/Economics/DailyTrafficVariation-States.html"><p>Click for direct access to the daily traffic dashboard </p></a></div>-->
<div class="card bg-light border-0">
<a href="https://www.eurocontrol.int/Economics/DailyTrafficVariation-States.html">
<img class="card-img-top" src="/images/landing/daily_db.png" alt="">
</a>
</div>
</div>
</div>
</div>

<!-- script for traffic figures -->
<script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>

<script type="text/javascript">
async function fetchLatestNetworkSituation() {
    const apiUrl = 'https://aiu-portal.pockethost.io/api/collections/network_situation/records?sort=-date&perPage=1&skipTotal=1';
    try {
        const response = await fetch(apiUrl);
        if (!response.ok) {
            throw new Error(`API call failed: ${response.status}`);
        }
        const data = await response.json();
        return data.items[0]; // Assuming the latest entry is what we need
    } catch (error) {
        console.error("Failed to fetch latest network situation:", error);
        // Implement any fallback logic or error handling here
        return null;
    }
}

async function updateNetworkSituationOnPage() {
    const latestData = await fetchLatestNetworkSituation();
    if (latestData) {
        // Extract the year from the latest data's date
        const latestDate = new Date(latestData.date);
        const year = latestDate.getFullYear();
        // Correctly construct the date range string
        const startDateString = `01 Jan`; // Fixed start of the year
        const endDateString = `${('0' + latestDate.getDate()).slice(-2)} ${latestDate.toLocaleString('default', { month: 'short' })} ${year}`;
        const dateRangeString = `${startDateString} – ${endDateString}`;
        // Update webpage elements with the latest data, rounding numbers to integers
        document.getElementById("y2d_day_text").innerHTML = "" + dateRangeString;
        document.getElementById("y2d_traffic").innerHTML = Math.round(latestData.y2d_flights_total).toLocaleString();
        document.getElementById("avg_y2d_traffic").innerHTML = Math.round(latestData.y2d_flights_daily_average).toLocaleString();
        document.getElementById("dif_y2d_2019").innerHTML = Math.round(latestData.y2d_diff_2019_year_percentage * 100) + '%';
        document.getElementById("dif_y2d_prev_year").innerHTML = Math.round(latestData.y2d_diff_previous_year_percentage * 100) + '%';
    } else {
        // Handle case when no data could be fetched
        console.log("No data available to update the network situation.");
    }
}
</script>
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function() {
        updateNetworkSituationOnPage();
    });
</script>
<br>
<div class="container">
<div class="row">
<div class="col-sm-8"><h4>Aviation Intelligence Portal</h4>

This portal is provided by the [Performance Review Unit (PRU)](/about/us/) which is part of EUROCONTROL's Aviation Intelligence Unit (AIU). 

In support of the independent work of the [Performance Review Commission (PRC)](/about/prc/) this portal aims to:

* ensure that all stakeholders in all [EUROCONTROL States](/definition/eurocontrol-area/) have high quality information on [ANS](/acronym/ans/) performance available in a transparent, accurate, complete and unbiased manner;
* stimulate an informed dialogue between all stakeholders for the benefit of the entire aviation network; and,
* further develop the [ANS](/acronym/ans/) performance analysis framework and the understanding of key [ANS](/acronym/ans/) performance issues.

For feedback or questions please contact us at:
[Performance Review Unit](mailto:PRU-Support@eurocontrol.int)
</div>


<div class="col-sm-4" style="min-height: 425px;"><h4>What's new?</h4><br> 
<div id="carouselExampleFade" class="carousel slide carousel-fade" data-ride="carousel">
<div class="carousel-inner">

<!--
<div class="carousel-item active"><div class="card bg-light border-0"><a href="/traffic/punctuality/"><img class="card-img-top" src="/images/landing/ectl_pct.png" alt=""><div class="card-body"><h5 class="card-title">Are flights punctual?</h5></a><small><p class="card-text">Check out the latest punctuality situation in Europe, including a breakdown by airport and state!</p></small></div></div></div>
-->

<div class="carousel-item active"><div class="card  border-0"><a href="https://eurocontrol.int/publication/prc-performance-insight-8-atm-performance-benefit-pools-improved-civil-military" target="_blank"><img class="card-img-top" src="/images/landing/pi8.png" alt=""><div class="card-body"><h5 class="card-title">PRC Performance Insight #8</h5></a><small><p class="card-text">
Learn more about ATM benefit pools for improved civil-military cooperation and coordination.</p></small></div></div></div>

<div class="carousel-item"><div class="card  border-0"><a href="https://apps.apple.com/tr/app/eurocontrol-data/id6467232405" target="_blank"><img class="card-img-top" src="/images/landing/eurocontrol-data.png" alt=""><div class="card-body"><h5 class="card-title">Released: EUROCONTROL DATA App</h5></a><small><p class="card-text">Download our new DATA app (on <a href="https://apps.apple.com/tr/app/eurocontrol-data/id6467232405">Apple Store</a> & <a href="https://play.google.com/store/apps/details?id=com.performancereviewunit.eurocontroldata">Google Play</a>) to access our unique aviation data.</p></small></div></div></div>

<div class="carousel-item"><div class="card  border-0"><a href="https://cadmus.eui.eu/handle/1814/76498" target="_blank"><img class="card-img-top" src="/images/landing/prc_tss2.PNG" alt=""><div class="card-body"><h5 class="card-title">PRC Transformation Support Strategy</h5></a><small><p class="card-text">
Learn more about the 2nd PRC workshop on innovative ATM transformation flagships in the new Florence School of Regulation (FSR) Policy Brief. </p></small></div></div></div>

<div class="carousel-item"><div class="card  border-0"><a href="https://www.eurocontrol.int/publication/comparison-air-traffic-management-related-operational-and-economic-performance" target="_blank"><img class="card-img-top" src="/images/landing/us_europe_2024.PNG" alt=""><div class="card-body"><h5 class="card-title">U.S.-Europe Comparison of ATM Performance Report 2024 </h5></a><small><p class="card-text">
Read the new US-Europe 2024 comparison report of ATM related operational and economic performance. </p></small></div></div></div>

<div class="carousel-item"><div class="card  border-0"><a href="/data/"><img class="card-img-top"  src="/images/landing/data_icon.jpg" alt="Data update"><div class="card-body"><h5 class="card-title">Performance data update</h5></a><small><p class="card-text">January 2024 data available for download now!</p></small></div></div></div>

<!--
<div class="carousel-item"><div class="card bg-light border-0"><a href="https://www.eurocontrol.int/publication/eurocontrol-european-aviation-overview" target="_blank" ><img class="card-img-top" src="/images/landing/eao.png" alt=""><div class="card-body"><h5 class="card-title">European Aviation Overview</h5></a><small><p class="card-text">
Read our EUROCONTROL aviation briefings on the latest trends in the European network.</p></small></div></div></div>

<div class="carousel-item"><div class="card bg-light border-0"><a href="/prc/transformation/"><img class="card-img-top" src="/images/landing/prc_tss.png" alt=""><div class="card-body"><h5 class="card-title">Transformation</h5></a><small><p class="card-text">
Learn about the PRC's strategy to support the transformation process in ANS.</p></small></div></div></div>

-->

</div>

<a class="carousel-control-prev" href="#carouselExampleFade" role="button" data-slide="prev"><span class="carousel-control-prev-icon" aria-hidden="true"></span><span class="sr-only">Previous</span></a>
<a class="carousel-control-next" href="#carouselExampleFade" role="button" data-slide="next"><span class="carousel-control-next-icon" aria-hidden="true"></span><span class="sr-only">Next</span></a>
</div>
</div>

<!-- 
<div class="col-sm-2"><h4></h4><br>
<div class="card bg-light text-center border-0"><a href="https://www.eurocontrol.int/Economics/DailyTrafficVariation-States.html">
  <img class="card-img-top" src="/images/landing/ectl_int_db.png" alt="Daily traffic statistics"><br>
  <div class="card-body"><h6 class="card-title">Performance Snapshot</h6></a><small><p class="card-text">Click to see the latest monthly snapshot!</p></small>
  </div>
</div>
</div>-->

</div>

#### Focus areas
<!-- cards with Key Performance Areas -->
<div class="container">
<div class="row">
<div class="card-deck">
<div class="card"><a href="/traffic">
  <img class="card-img-top" src="/images/landing/ectl_traff_th.jpg" alt="Traffic">
  <div class="card-body"><h6 class="card-title">Traffic</h6></a><small><p class="card-text">[IFR](/acronym/ifr/) flight and [service unit](/acronym/su/) evolution and forecast </p></small>
  </div>
</div>
<div class="card"><a href="/capacity">
  <img class="card-img-top" src="/images/landing/ectl_cap_th.jpg" alt="Capacity">
  <div class="card-body"><h6 class="card-title">Capacity & Delays</h6></a><small><p class="card-text">En-route and airport [ATFM](/acronym/atfm/) delays</p></small>
  </div>
</div>
<div class="card"><a href="/efficiency">
  <img class="card-img-top" src="/images/landing/ectl_env_th.jpg" alt="Efficiency & Environment">
  <div class="card-body"><h6 class="card-title">Efficiency & Environment</h6></a><small><p class="card-text">Flight efficiency affecting fuel burn and emissions</p></small>
  </div>
</div>
<div class="card"><a href="/economics">
  <img class="card-img-top" src="/images/landing/ectl_cef_th.jpg" alt="Economics">
  <div class="card-body"><h6 class="card-title">Economics</h6></a><small><p class="card-text">Analysis of ANS cost-efficiency</p></small>
  </div>
</div>
</div>
</div>
</div>

<br>
<div class="container text-center">
{{% subscribe-button %}}

[Read our privacy protection statement](/about/privacy/)
</div>

[ectl_daily]: https://www.eurocontrol.int/Economics/DailyTrafficVariation "Daily traffic updates"
[ectlweb]: https://www.eurocontrol.int/ "EUROCONTROL web page"