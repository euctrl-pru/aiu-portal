...
This dashboard provides historical estimates of CO<sub>2</sub>, NO<sub>X</sub>, and SO<sub>X</sub> emissions based on the **Gate-to-Gate methodology**, developed jointly by the [EUROCONTROL Performance Review Unit](https://www.eurocontrol.int/pru) and the [EUROCONTROL Aviation Sustainability Unit](https://www.eurocontrol.int/aviation-sustainability){target="_blank"}.

Emissions are calculated using the [Advanced Emissions Model (AEM)](https://www.eurocontrol.int/tool/advanced-emission-model), and are based on the best available trajectory data. The data reflects results from the **Area Focus methodology**, as presented on the [EUROCONTROL Flying Green platform](https://flyinggreen.eurocontrol.int/). For details on the methodology, data sources, and intended use, please refer to the [G2G Methodology Info](/reference/dataset/g2g-emissions/).
...
<div class="resp-container">
<iframe class="resp-iframe" src="https://app.powerbi.com/view?r=eyJrIjoiN2RlNmU5NTMtMGE0OS00ZjA3LTk3MDAtN2YwNTMwMzkxMjg1IiwidCI6Ijc2ZjMzYzIwLTU5NzktNDQwOC1hZGY3LThiM2M0YmU5NWU1MiIsImMiOjl9&pageName=bd3b3877791821bcf5d1" frameborder="0" style="border:0" allowfullscreen></iframe>
</div>
</div>
</div>


To view the dashbboards in full-screen mode, [click here](https://app.powerbi.com/view?r=eyJrIjoiN2RlNmU5NTMtMGE0OS00ZjA3LTk3MDAtN2YwNTMwMzkxMjg1IiwidCI6Ijc2ZjMzYzIwLTU5NzktNDQwOC1hZGY3LThiM2M0YmU5NWU1MiIsImMiOjl9).
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (hash) {
    ...
    if (tabLink) {
      // Remove 'active' class from any currently active tab and pane
      document.querySelectorAll('.nav-link.active, .tab-pane.show.active').forEach(el => {
        el.classList.remove('active', 'show');
      });

      // Add 'active' class to the new tab and pane
